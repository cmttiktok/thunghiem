...
// --- CẤU HÌNH GEMINI AI ---
// Bạn nhớ thêm GEMINI_API_KEY vào Environment Variables trên Render nhé
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
const aiModel = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

// --- HÀM LẤY AUDIO GOOGLE (DẠNG BASE64 ĐỂ TRÁNH LỖI CORS) ---
...
io.on('connection', (socket) => {
    ...
    socket.on('set-username', (username) => {
        ...
        tiktok.on('chat', async (data) => {
            try {
                // 1. Kiểm tra từ cấm (Silent filter)
                const banned = await BannedWord.find();
                if (banned.some(b => data.comment.toLowerCase().includes(b.word))) return;

                // 2. Lấy Prompt xưng hô Mina từ Database
                const promptDoc = await Config.findOne({ key: 'prompt' });
                const sysPrompt = promptDoc ? promptDoc.value : "Bạn là Mina, trợ lý ảo lém lỉnh.";

                // 3. Gọi Gemini AI - Truyền tên người dùng vào nội dung để AI chào đúng tên
                const userMessage = `Người dùng tên "${data.nickname}" nói: ${data.comment}. Hãy phản hồi họ theo phong cách của bạn.`;
...
server.listen(PORT, () => console.log(`🚀 Hệ thống Mina khởi động tại Port ${PORT}`));
